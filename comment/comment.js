...
class Comment {
	...
	async renderCommentsHtml(node, from) {
		...
		switch (from) {
			...
			case "block":
              ...
              <div class="list-item">
                <div class="header">
                  <div class="date">${formatSYDate(
										comments[key]["created"]
									)}</div>
                  <div class="actions">
                    <div class="delete-comment" data-quote-id="${quoteId}" data-comment-id="${
								comments[key]["block_id"]
							}">移除批注</div>
                    <div class="delete-comment" data-quote-id="${quoteId}" data-comment-id="${
								comments[key]["block_id"]
	...
	async submitComment() {
		...
		if (quoteId) {
			//追加批注
			let blockId = document.querySelector(
				`.protyle-wysiwyg [custom-${quoteId}]`
			).dataset.nodeId; //comment 所在 block
			let quoteText = document.querySelector(
				`strong[style*="quote-${quoteId}"]`
			).innerText;
			...
			while (
				start != null &&
				(start.dataset == null || start.dataset.nodeId == null)
			) {
	...
	async appendBlocks(quoteText, blockId, quoteId) {
		let activeEditor =
			...
			document.querySelector('.layout__center [data-type="wnd"]') ||
			document.getElementById("editor"); //获得当前光标所在页面
		let background =
...
`;
		...
		// let existQuote = activeEditor.querySelector(
		...
		if (target.className == "delete-quote") {
			...
			let quoteId = target.dataset.quoteId,
				...
				block = document.querySelector(
					`.protyle-wysiwyg [data-node-id][custom-${quoteId}]`
				);
	...
	showBox(e) {
		...
		if (
			(target != null &&
				target.dataset != null &&
				target.dataset.type == "comment") ||
			(parent != null &&
				parent.dataset != null &&
				parent.dataset.type == "comment") ||
			(grandParent != null &&
				grandParent.dataset != null &&
				grandParent.dataset.type == "comment")
		) {
			// 1)点击 toolbar 图标触发
			e.stopPropagation();
			let selection = getSelection(),
			...
			if (range) {
				...
				if (
					start.parentElement.tagName == "STRONG" ||
					end.parentElement.tagName == "STRONG"
				) {
			...
			(target.classList &&
				target.classList.contains("protyle-attr--comment")) ||
			(parent.classList &&
				parent.classList.contains("protyle-attr--comment")) ||
			(grandParent.classList &&
				grandParent.classList.contains("protyle-attr--comment"))
		) {
			// 3)点击内容块右侧图标触发
			e.stopPropagation();
			show = true;
